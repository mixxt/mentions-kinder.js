/*! mentions-kinder - v0.1.0-beta8 - 2013-11-18
...
MentionsKinder = (function() {
  ...
  MentionsKinder.prototype.deserializeFromInput = function() {
    this.$editable.html(this._deserialize(this.$originalInput.val()));
  ...
  MentionsKinder.prototype.handleReset = function() {
    ...
    return this.$input.val(this.$originalInput.val());
  };
    ...
    if (e.type === 'focus') {
      if (!this.placeholderDetached) {
        return window.setTimeout(function() {
          ...
          _this.placeholderDetached = true;
          textNode = document.createTextNode('');
          return _this.$editable.html(textNode).focus();
        });
      }
  ...
  MentionsKinder.prototype._setupElements = function() {
    ...
    this.$input.attr('name', this.$originalInput.attr('name'));
    this.$input.val(this.$originalInput.val());
    ...
    if (placeholder = this.$originalInput.attr('placeholder')) {
      this.$placeholder = $("<span class='placeholder'>" + placeholder + "</span>").appendTo(this.$editable);
    }
    this.$wrap.insertAfter(this.$originalInput);
    this.$originalInput.hide().appendTo(this.$wrap);
    this.$input.appendTo(this.$wrap);
  ...
  MentionsKinder.prototype._setupEvents = function() {
    ...
    this.$originalInput.bind('change', this.deserializeFromInput);
    if (form = this.$originalInput.get(0).form) {
      ...
      return this.$form.on('reset', this.handlePlaceholder);
    }
  };
...
MentionsKinder.prototype.triggerDefaultOptions.autocompleter = Autocompleter.Select2Autocompleter;
